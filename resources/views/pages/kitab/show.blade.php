...
@section('title', $kitab->name . ' - ' . site_name())

@section('og_title', $kitab->name . ' - ' . site_name())
@section('og_description', $kitab->description ?: 'Kumpulan maqolah-maqolah hikmah dari ' . $kitab->name)
        ...
        <div class="bg-gradient-to-br from-primary via-gray-800 to-gray-900 text-white pt-8 pb-12 px-4 relative overflow-hidden">
            <div class="relative z-10 text-center max-w-7xl mx-auto">
                ...
                <div class="text-xs text-gray-400 mb-4">
                    <a href="{{ route('home.kitab.index') }}" class="hover:text-white transition-colors">
                        <i class="fas fa-book-quran mr-1"></i> Koleksi Kitab
                    </a>
                    <i class="fas fa-chevron-right mx-2 text-[10px]"></i>
                    <span class="text-white">{{ $kitab->name }}</span>
                </div>
                ...
                <h1 class="text-2xl font-bold mb-2 tracking-tight">{{ $kitab->name }}</h1>
                @if($kitab->author)
                <p class="text-sm text-secondary mb-2 font-medium">{{ $kitab->author }}</p>
                @endif
                @if($kitab->description)
                <p class="text-xs text-gray-400 max-w-sm mx-auto leading-relaxed">
                    {{ Str::limit(strip_tags($kitab->description), 150) }}
                </p>
                @endif
                <div class="flex justify-center gap-4 mt-6 text-xs mb-4">
                    ...
                    <div class="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/10">
                        <i class="fas fa-scroll mr-1.5 text-secondary"></i> {{ $totalMaqolah }} Maqolah
                    </div>
                </div>

                {{-- Download for Offline Button --}}
                <div id="offline-download-wrapper" class="mb-8" data-kitab-slug="{{ $kitab->slug }}" data-total-maqolah="{{ $totalMaqolah }}" data-latest-update="{{ $latestUpdate }}">
                ...
                <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="flex items-center p-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-semibold text-gray-800 text-sm group-hover:text-primary transition-colors line-clamp-1 mb-1">
                                        {{ $kitab->name }} Bab {{ $chapter->nomor_bab }}{{ $chapter->judul_bab ? ' : ' . $chapter->judul_bab : '' }}
                                    </h3>
                    ...
                    @empty
                        <div class="col-span-full text-center py-10 text-gray-500 bg-white rounded-xl border border-dashed border-gray-300">
                            ...
                            <p class="text-lg font-semibold">Belum ada bab dalam kitab ini.</p>
                        </div>
                    @endforelse
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        chapterSelect.addEventListener('change', function() {
            ...
            if (maqolahs.length > 0) {
                maqolahs.forEach(m => {
                    const label = m.judul ? `Maqolah ${m.nomor_maqolah}: ${m.judul}` : `Maqolah ${m.nomor_maqolah}`;
        ...
        maqolahSelect.addEventListener('change', function() {
            ...
            btnGo.disabled = !currentMaqolahId;
        });

        // Handle Go Click
        btnGo.addEventListener('click', function() {
            if (currentChapterSlug && currentMaqolahId) {
                // Construct URL: /kitab/{kitabSlug}/{chapterSlug}/maqolah/{id}
                const url = `/kitab/${kitabSlug}/${currentChapterSlug}/maqolah/${currentMaqolahId}`;
        ...
        const currentKitabSlug = wrapper.dataset.kitabSlug;
        ...
        // Get stored data from localStorage (per-kitab)
        const storageKey = `kitab_offline_${currentKitabSlug}`;
        ...
        // Determine if there's any update
        ...
        if (wasDownloaded && !needsUpdate) {
            ...
            updateNotification.innerHTML = '<i class="fas fa-info-circle mr-1"></i> Klik untuk repair/download ulang jika ada masalah.';
        ...
        async function startDownload() {
            ...
            try {
                ...
                const response = await fetch(`/kitab/api/urls?kitab=${currentKitabSlug}`);
